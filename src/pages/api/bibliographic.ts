...
export default async function handler(
  ...
  try {
    ...
    if (id) {
      let response = await getBibliographicItem(
        id,
        baseUrl,
        apiKey,
        lang ? lang : "en"
      ...
      let response = await getBibliographic(
        query,
        count,
        q,
        start,
...
async function getBibliographicItem(
  id: string,
  baseUrl: string,
  apiKey: string,
  lang: string
) {
  baseUrl += `bibliographic/v1/${id}/?format=json&lang=${lang}`;
  const response = await axios.get(baseUrl, { headers: { apiKey: apiKey } });
  return response;
}
async function getBibliographic(
  query: string,
  count: number,
  q: string,
  start: number,
